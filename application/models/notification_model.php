...
for($i = 0 ; $i <= 3 ; $i++){    
    ...
    $query = $this->db->query("select ".$arrayName[$i]." as name, ".$arrayType[$i]." as type ,sup_company as supplier, ".$arrayStock[$i]." as stock, ".$arrayreorder[$i]." as reorder from ".$arrayTable[$i]." join supplier using(sup_id) where sup_activation = 1 and ".$arrayStock[$i]." <= ".$arrayreorder[$i]);
